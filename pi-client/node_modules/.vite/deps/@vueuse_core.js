import { $n as toRefs$1, An as customRef, At as onBeforeMount, Bn as markRaw, Bt as onUpdated, Ft as onMounted, Gn as readonly, In as isReactive, Kn as ref, Ln as isReadonly, M as Fragment, Mn as effectScope, Mt as onBeforeUpdate, Nn as getCurrentScope, Ot as nextTick, Qn as toRef$1, Rn as isRef, U as computed, Ut as provide, Vn as onScopeDispose, Wn as reactive, Yn as shallowRef, Zn as toRaw, _n as watchEffect, gn as watch, jt as onBeforeUnmount, nr as unref, nt as defineComponent, pt as h, qn as shallowReactive, r as TransitionGroup, ut as getCurrentInstance, xt as inject, zt as onUnmounted } from "./vue.runtime.esm-bundler-CnFIjQfI.js";
import "./lib-ChgqNE6I.js";
...
function useBreakpoints(breakpoints, options = {}) {
	...
	return Object.assign(shortcutMethods, {
		...
		isGreater(k) {
			return match(`(min-width: ${getValue(k, .1)})`);
		},
		isGreaterOrEqual(k) {
			return match(`(min-width: ${getValue(k)})`);
		},
		isSmaller(k) {
			return match(`(max-width: ${getValue(k, -.1)})`);
		},
		isSmallerOrEqual(k) {
			return match(`(max-width: ${getValue(k)})`);
		},
		isInBetween(a, b) {
			return match(`(min-width: ${getValue(a)}) and (max-width: ${getValue(b, -.1)})`);
		},
		current,
		active() {
			const bps = current();
...
function useDark(options = {}) {
	...
	const system = computed(() => {
		...
		else return usePreferredDark({ window }).value ? "dark" : "light";
	});
	return computed({
		get() {
...
function useDropZone(target, options = {}) {
	...
	if (isClient) {
		...
		const checkValidity = (event) => {
			...
			const dataTypesValid = checkDataTypes(items.map((item) => item.type));
		...
		const handleDragEvent = (event, eventType) => {
			...
			switch (eventType) {
				case "enter":
					...
					(_a2 = _options.onEnter) == null || _a2.call(_options, null, event);
					break;
				case "over":
					(_b2 = _options.onOver) == null || _b2.call(_options, null, event);
					break;
				case "leave":
					counter -= 1;
					if (counter === 0) isOverDropZone.value = false;
					(_c = _options.onLeave) == null || _c.call(_options, null, event);
					break;
				case "drop":
					counter = 0;
					...
					if (isValid) {
						...
						(_d = _options.onDrop) == null || _d.call(_options, currentFiles, event);
					}
					break;
			}
		};
...
function useMouse(options = {}) {
	...
	if (target) {
		...
		if (scroll && type === "page") useEventListener(window, "scroll", scrollHandlerWrapper, { passive: true });
	}
	return {
		x,
		y,
...
function checkPassiveEventSupport(document) {
	...
	document.addEventListener("x", noop, { get passive() {
...
function useUrlSearchParams(mode = "history", options = {}) {
	const { initialValue = {}, removeNullishValues = true, removeFalsyValues = false, write: enableWrite = true, window = defaultWindow } = options;
